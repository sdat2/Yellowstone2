...
filt= 'tff3'
Fail_Count= {'Range':0,'fitid':0,'Noise_before':0,'Noise_after':0,'Minamp':0}

fitmin = 70 # Minimum 60 percent of radial compoment should be fit (after reconvolving the RF with the vertical component
noisebefore = 0.3 # There should be no peaks before the main P wave which are more than 30% of the amplitude of the Pwave
noiseafter = 0.5# There should be no peaks after the main P wave which are more than 70% of the amplitude of the Pwave
                        ...
                        if np.max(np.abs(RF.data[0:indm-40]))<noisebefore:
                            if np.max(np.abs(RF.data[indm+40:indm+1200]))<noiseafter:
                                if np.max(np.abs(RF.data[indm+20:-1]))>minamp:
                                    print('good station')
                                    shutil.move(stalist[i],new_direc+stalist[i][file_char_len:])
                                else:
                                    Fail_Count['Minamp']+=1
